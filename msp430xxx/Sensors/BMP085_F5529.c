...
{	sensor_BMP085 *temp;
	...
	I2C0_SetSlaveAdress(0x77);				// set BMP085 hardware I2C adress
...
void BMP_calibrate(sensor_BMP085 *sensor){
    I2C0_SendAndRead(BMP_AC1, 22, (unsigned short *)val);	// read all 22 calibration bytes, starting at adress 0xAA
	sleep(400);			// give enough time to read
...
void BMP_measure(sensor_BMP085 *sensor) {
	...
	I2C0_SetSlaveAdress(sensor->adress);		// make sure correct I2C adress is set
	I2C0_WriteRegister(BMP_MEAS_MODE, BMP_MEAS_TEMP);	// write value for temperatrue measurement into measure mode register on sensor
	sleep(20);			// wait at least 4.5 ms for sensor to finish conversion
	I2C0_SendAndRead(BMP_MEAS_MSB, 2, (unsigned short*)val); // set sensor internal pointer, then read 2 values
	sleep(20);			// wait for I2C communication
	sensor->uT = append(val[0], val[1]);

	I2C0_WriteRegister(BMP_MEAS_MODE, BMP_MEAS_PRESS+(sensor->oss<<6));	// write value for pressure measurement into measure mode register on sensor
	sleep(200);			// wait at least 16 ms in oss=3 for sensor to finish conversion
	I2C0_SendAndRead(BMP_MEAS_MSB, 3, (unsigned short*)val); // set sensor internal pointer, then read 2 values
	sleep(20);			// wait for I2C communication to finish
