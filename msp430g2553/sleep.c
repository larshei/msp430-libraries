...
void sleep(unsigned int time, int secOrTenthMillisec){
	int _clockDivider = 0, _clockSourceBits;

	// set correct clock source
	switch (_clockSource)
	{	case TACLK:
			_clockSourceBits = TASSEL_0;
			break;
		case ACLK:
			_clockSourceBits = TASSEL_1;
			break;
		case SMCLK:
			_clockSourceBits = TASSEL_2;
			break;
		case INCLK:
			_clockSourceBits = TASSEL_3;
			break;
		default:			// no correct value has been set -> use SMCLK because always available
			_clockSourceBits = TASSEL_2;
			break;
	}
	...
	__enable_interrupt();
	TA1CTL = TACLR;
	TA1CTL = MC_2 + TAIE + _clockDivider + _clockSourceBits;	// cont mode timer (always count to 0xFFFF), enable interrupt
